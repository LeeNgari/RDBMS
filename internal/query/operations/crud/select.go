...
func SelectAll(table *schema.Table, proj *projection.Projection) []data.Row {
	table.RLock()
	defer table.RUnlock()
...
func SelectWhere(table *schema.Table, pred PredicateFunc, proj *projection.Projection) []data.Row {
	table.RLock()
	defer table.RUnlock()
	...
	for _, row := range table.Rows {
		if pred(row) {
			result = append(result, projection.ProjectRow(row, proj, table.Name))
		}
	}
	return result
}
...
func SelectByUniqueIndex(table *schema.Table, colName string, value interface{}, proj *projection.Projection) (data.Row, bool) {
	table.RLock()
	defer table.RUnlock()
	...
	return projection.ProjectRow(row, proj, table.Name), true
}
